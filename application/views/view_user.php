            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php $membership = $this->db->get_where('user_groups', array('uid'=>$user_id, 'gid'=>$group['id']))->row();
                     if(!$membership){?>
                   Not a Member <a href="#"  onclick="addToGroup()" data-id="<?=base_url()?>add/<?= $user->id.'/'.$group['id'] ?>" class="btn btn-primary add-to-group">Add to Group</a>
                     <?php } else {?>
...
<a href="<?=site_url()?>/users">Go Back</a>
